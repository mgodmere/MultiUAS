...
gimbal_udp = 20400 # typhoon gimbal stream port
...
def main():
	...
	typhoon_template = '''
		...
		mavlink stream -r 250 -s HIGHRES_IMU -u {}
		mavlink stream -r 10 -s OPTICAL_FLOW_RAD -u {}
		mavlink stream -r 10 -s MOUNT_ORIENTATION -u {}
		mavlink stream -r 50 -s ATTITUDE_QUATERNION -u {}
		mavlink stream -r 10 -s MOUNT_ORIENTATION -u {}
		logger start -e -t
		vmount start
	...
	for i in range(1, int(sys.argv[1]) + 1):
		...
		if platform == 'iris':
		...
		elif platform == 'typhoon_h480':
			vehicle_config = open('Firmware/posix-configs/SITL/init/ekf2/typhoon_h480_' + str(i), 'w')
			vehicle = typhoon_template.format(
				i,
				mav_udp + i,
				gcs + i,
				fcu_s + i,
			...
			raise Exception('Unsupported vehicle type: ' + platform)
